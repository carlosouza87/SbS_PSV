...
Jth = zeros(6,6,2);
for k1 = 1:2
    phi = eta(4+(k1-1)*6,1); %(k1-1)*6 é a notação para pegar as linhas da segunda embarcação quando k1 for 2.
...
eta1p = Jth(:,:,1)*nu(1:6,1);%conversão da aceleração de um referencial para outro.
eta2p = Jth(:,:,2)*nu(7:12,1);
etap = [eta1p;eta2p];
% Geta = [Jth(:,:,1)\G(1:6,1:6)*eta(1:6);Jth(:,:,2)\G(7:12,7:12)*eta(7:12)];
nup = M\(-(Crb+Ca)*nu-B*(nu-nu_m)-G*eta+tau);%a aceleração em eta, efeitos hidrodinâmicos não estão funcionando
